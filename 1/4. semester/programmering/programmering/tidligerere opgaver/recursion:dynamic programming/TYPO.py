...
def mindist(a,b):
     ...
     if not b:
          return len(a)*5
     return min([manhat(a[0],b[0])+mindist(a[1:],b[1:]),5+mindist(a[1:],b),5+mindist(a,b[1:])])


wdist={b:mindist(word,b) for b in bib}
mdi=min(wdist[b] for b in wdist)
bwords=[w for w in wdist if wdist[w]==mdi]
print(mdi)
for c in bwords:
     print(c)