...
def print_stars(stars,alphabetically=False,highscore=False,vertical=False):
    ...
    if vertical:
        st=[["_" for _ in range(len(names))] for _ in range(mstar)]
        for i in range(len(stars)):
            for j in range(stars[i][1]):
                st[-j-1][i]="*"
        for lin in st:
            print("_".join(lin))
        te=[["_" for _ in range(len(names))] for _ in range(mlen)]
        for i in range(len(stars)):
            ind=0
            it=iter(stars[i][0])
            try:
        ...
        for k in stars:
            print(k[0]+(mlen+1-len(k[0]))*"_"+int(k[1])*"*")
